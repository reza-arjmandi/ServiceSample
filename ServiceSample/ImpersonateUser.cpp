...
bool ImpersonateUser::Logon(const std::string& userName, const std::string& domain, const std::string& password)
{
	...
	if(domain.length() > 0) // Domain name was specified
		bLoggedOn = LogonUserA(userName.c_str(), domain.c_str(), password.c_str(), LOGON32_LOGON_INTERACTIVE, LOGON32_PROVIDER_DEFAULT, &userToken_);
	else
		bLoggedOn = LogonUserA(userName.c_str(), domain.c_str(), password.c_str(), LOGON32_LOGON_NEW_CREDENTIALS, LOGON32_PROVIDER_WINNT50, &userToken_);
